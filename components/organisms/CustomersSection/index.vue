...
export default {
    ...
    methods: {
        ...
        processDataCustomers(customers, provider) {
            ...
            switch (provider) {
                case 'netflix':
                const netArr = theArr.slice(theArr.length-4, theArr.length-0).sort().reverse()
                this.netflixCustomers = netArr;
                    break;
                case 'spotify':
                const spoArr = theArr.slice(theArr.length-4, theArr.length-0).sort().reverse()
                this.spotifyCustomers = spoArr;
                    break;
                case 'gramedia':
                const graArr = theArr.slice(theArr.length-4, theArr.length-0).sort().reverse()
