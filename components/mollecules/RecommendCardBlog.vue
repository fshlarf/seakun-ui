<template>
  <div class="flex w-full cursor-pointer items-center">
    <img
      :class="`${imageSize} rounded-[6px] object-cover`"
      :src="`${article.image}`"
    />
    <div class="w-full pl-[12px]">
      ...
      <!-- Title -->
      <div :class="`${titleSize} mb-1 font-semibold text-main`">
        {{ article.title }}
      </div>
      ...
      <div class="flex items-center justify-between">
        ...
        <template v-if="showDate">
          <div class="flex items-center">
            ...
            <p
              :class="`${getColor} ${padding} text-[10px] text-[#66738F] md:text-xs`"
            >
              {{ article.created_at }}
            </p>
          </div>
      ...
      <template v-if="showAuthor">
        <p class="text-sm font-normal text-[#A0A3BD]">{{ article.author }}</p>
      </template>
    </div>
  </div>
</template>
