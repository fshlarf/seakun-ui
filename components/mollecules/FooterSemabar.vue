<template>
  <div class="footer lg:pt-6 xl:pt-12 mt-12">
    <div
      class="container-semabar text-white py-4 md:grid md:grid-cols-2 md:justify-between lg:grid-cols-5 lg:gap-8 lg:justify-center"
    >
...
export default {
  ...
  methods: {
    scrollTo(id) {
      const currentPath = this.$nuxt.context.route.path;
      const segments = currentPath.split('/');
      const lastSegment = segments.slice(1).join('/');
      const navbarElement = document.getElementById('navbar');
      if (lastSegment == 'semabar' && navbarElement) {
