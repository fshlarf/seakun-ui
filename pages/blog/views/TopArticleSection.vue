<template>
  ...
  <div class="w-full lg:w-1/2">
    <div class="title-font">
      <label class="text-[16px] font-bold text-text-dark">Top</label>
      <label class="text-[16px] font-bold text-primary"> Article</label>
    </div>
    <div class="mt-[20px] grid grid-cols-2 gap-5">
      <div v-for="(article, id) in topPaginated" :key="id">
        <CardTop
          show-label
          show-date
          card-type="top"
...
<script>
import CardTop from '~/components/mollecules/ArticleCardBlog.vue';
...
export default {
  ...
  computed: {
    topPaginated() {
      ...
      return this.top_article.slice(start, end);
    },
    total() {
      return Math.ceil(this.top_article.length / this.limit);
    },
  },
  methods: {
    prevPage() {
