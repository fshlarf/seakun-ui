        ...
        <div class="hidden lg:block mt-8 w-[352px]">
          <div class="border rounded-[8px] p-4">
            ...
            <div
              v-for="(info, id) in priceScheme.informations"
              :key="id"
              class="flex items-center justify-between text-sm mt-1 text-main"
              :class="`${
                id == priceScheme.informations.length - 1 || id == 5
                  ? 'font-bold'
                  : ''
              }`"
            >
              <p class="max-w-[209px]">{{ info.title }}</p>
              <p>{{ info.value }}</p>
            </div>
          </div>
    ...
    async createOrder({ commit, dispatch }, params) {
      ...
      try {
        ...
        if (fetchCreateOrder.data) {
          ...
          console.log('try');
          this.$router.push(`/seglowup/thankyou`);
          console.log('finish');
        } else {
